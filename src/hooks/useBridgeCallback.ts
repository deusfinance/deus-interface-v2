...
export function useMetaBridgeCallback(
  inputCurrency: Currency,
  inputAmount: CurrencyAmount<NativeCurrency | Token> | null | undefined
): {
  state: TransactionCallbackState
  ...
  const constructCall = useCallback(() => {
    try {
      if (!account || !library || !MetaBridgeGatewayContract || !inputAmount) {
        throw new Error('Missing dependencies.')
      }

      const methodName = inputCurrency?.symbol === 'EERC' ? 'swapToBridged' : 'swapToReal'

      const args = [toHex(inputAmount.quotient)]

      return {
        address: MetaBridgeGatewayContract.address,
        calldata: MetaBridgeGatewayContract.interface.encodeFunctionData(methodName, args) ?? '',
        value: 0,
      }
    } catch (error) {
      return {
  ...
  }, [account, library, MetaBridgeGatewayContract, inputAmount, inputCurrency?.symbol])

  return useMemo(() => {
    if (!account || !chainId || !library || !MetaBridgeGatewayContract || !inputCurrency) {
      return {
        state: TransactionCallbackState.INVALID,
        callback: null,
        error: 'Missing dependencies',
  ...
  }, [account, chainId, library, MetaBridgeGatewayContract, inputCurrency, inputAmount, constructCall, addTransaction])
}
